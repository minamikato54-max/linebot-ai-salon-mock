...
- **オーナー様への未対応質問通知の仕組みについて回答**
  - 会話ログ画面の「要対応」バッジで確認できることを説明
  - オーナー様ご本人への通知を有効にする手順（公式アカウントを友だち追加→メッセージ送信→line_user_idを`OWNER_LINE_USER_ID`に設定）を説明
- **LINE Developers ConsoleのWebhook URLを本番URLに更新**（ユーザーが実施・検証成功を確認済み）
- **ポートフォリオ用にGitHubリポジトリへ本格push**
  - `webapp`用のGitHubリポジトリ（`linebot-ai-salon-mock`）は作成済みだったが、実装コミットが未pushだったため全履歴をpush
  - ルートに`README.md`を新規作成（プロジェクト概要・できること・使用技術・スクリーンショット・公開URL・ドキュメント一覧）
  - `.gitignore`での秘密情報除外、コミット履歴の質を確認（問題なし）
- **引き継ぎ用ドキュメント3点を作成**（`webapp/docs/`配下）
  - `setup-guide.md`: セットアップ手順書（開発環境構築マニュアル）。スクリーンショット6箇所すべて埋め込み完了
  - `owner-manual.md`: 運用マニュアル（オーナー向け、スマホで読める平易な言葉で作成）
  - `technical-doc.md`: 技術ドキュメント（API仕様・DB設計・RLS・認証の仕組み・外部サービス連携のシーケンス図など）
- **スクリーンショット作業時に、実際の秘密情報が写り込んでいた画像を発見・対応**
  - LINEチャネルシークレットの値がそのまま写っていたスクリーンショットを黒塗り加工してから使用
  - ブラウザ/サイトのプロフィール写真アイコンが写っていた画像3枚もマスク処理（ユーザーからの指摘を受けて対応）
  - Supabaseの新しいAPI Keys画面はPublishable key（公開可）とSecret key（マスク済み）の両方を確認した上でそのまま使用

### 次にやること

- 運用マニュアル・README用に、管理画面ホーム画面のスクリーンショットが未反映（元ファイルが端末から見当たらず再送待ち）
- README用に、LINEでのbot応答画面・お知らせ配信の確認ステップ画面のスクリーンショットが未反映
- いずれも無くても各ドキュメントとして成立する内容のため、優先度は低め
...
- Read（画像読み込みによるスクリーンショット内容確認・秘密情報チェック）
- Bash + Python(PIL)（スクリーンショット内の秘密情報・個人情報の黒塗り加工）
- git（GitHubへのpush、コミット履歴・.gitignoreの確認）
